...
class Friend {
	...
	sendFriendRequest = async (candidateFriendId) => {
		...
		try {
			const response = await axios.post(
				'http://localhost:5000/api/user/friend-requests/send',
	...
	acceptFriendRequest = async (candidateFriendId) => {
		...
		try {
			const response = await axios.patch(
				'http://localhost:5000/api/user/friend-requests/accept',
				{ candidateFriendId, userId },
				{
					headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
				}
	...
	declineFriendRequest = async (candidateFriendId) => {
		try {
			const userId = user.id
			const response = await axios.patch(
				'http://localhost:5000/api/user/friend-requests/decline',
				{ candidateFriendId, userId },
				{
					headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
				}
	...
	removeFriend = async (candidateFriendId) => {
		try {
			const userId = user.id
			const response = await axios.patch(
				'http://localhost:5000/api/user/friend/remove',
				{ candidateFriendId, userId },
				{
					headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
				}
...
dRequestList = async () => {
		...
		try {
			const response = await axios.get(
				'http://localhost:5000/api/user/friends/requests',
				{
					params: { userId },
					headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
				}
	...
	getFriend = async (friendId) => {
		try {
			const response = await axios.get(
				'http://localhost:5000/api/user/friend',
				// {friendId},
				{
					params: { friendId },
					headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
